...
impl Otoroshi {
    ...
    fn get_string_from_ptr(ptr: u64) -> Option<String> {
        match Self::find_memory(ptr) {
            None => None,
            Some(mem_out) => match mem_out.to_string() {
                Err(_e) => None,
        ...
        match Self::get_json_object_from_ptr(ptr) {
            ...
            Some(mem) => {
                ...
                mem.iter().for_each(|entry| {
                    res.insert(
                        entry.0.to_owned(),
                        Self::decode_base64(entry.1.as_str().unwrap().to_owned()),
                    );
                });
                Some(res)
            }
    ...
    pub fn plugin_mem_get_json(key: &str) -> Option<serde_json::Value> {
        Self::plugin_mem_get_string(key).and_then(|s| match serde_json::from_str(s.as_str()) {
            Err(_e) => None,
            Ok(v) => Some(v),
        })
    }
    ...
    pub fn plugin_mem_get_bool(key: &str) -> Option<bool> {
        Self::plugin_mem_get(key).map(|bytes| match bytes.get(0) {
            Some(0) => false,
            Some(1) => true,
            _ => false,
        })
    ...
    pub fn shared_mem_get_json(key: &str) -> Option<serde_json::Value> {
        Self::shared_mem_get_string(key).and_then(|s| match serde_json::from_str(s.as_str()) {
            Err(_e) => None,
            Ok(v) => Some(v),
        })
    }
    ...
    pub fn shared_mem_get_bool(key: &str) -> Option<bool> {
        Self::shared_mem_get(key).map(|bytes| match bytes.get(0) {
            Some(0) => false,
            Some(1) => true,
            _ => false,
        })
    ...
    pub fn datastore_all_matching(pattern: &str) -> Vec<Vec<u8>> {
        ...
        match Self::get_json_array_from_ptr(ptr) {
            ...
            Some(arr) => arr
                .into_iter()
                .map(|x| Self::decode_base64(x.as_str().unwrap().to_string()))
                .collect(),
        }
    }

    pub fn datastore_all_matching_string(pattern: &str) -> Vec<String> {
        Self::datastore_all_matching(pattern)
            .iter()
    ...
    pub fn datastore_all_matching_json(pattern: &str) -> Vec<serde_json::Value> {
        Self::datastore_all_matching_string(pattern)
            .iter()
